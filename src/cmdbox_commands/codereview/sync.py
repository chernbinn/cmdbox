...
def sync_project(config):
    ...
    track_branch = config['track_branch']
    target_branch = config['target_branch']
    gerrit_remote = config['gerrit_remote']
    gerrit_branch = config['gerrit_branch']
    ...
    try:
        ...
        # 获取上次同步的commit
        last_commit = git.get_last_synced_commit(target_branch)
        if not last_commit:
            # 首次同步，使用 merge-base
            last_commit = git.get_merge_base(track_branch, target_branch)
            if last_commit:
                logger.info(f"\n首次同步，使用 merge-base: {last_commit[:7]}")
            else:
                logger.error(f"\n错误: 无法找到 {track_branch} 和 {target_branch} 的共同祖先")
                return False
        else:
            logger.info(f"\n上次同步的commit: {last_commit[:7]}")
        
        # 检查当前分支
        current_branch = git.get_current_branch()
        if current_branch != target_branch:
            logger.info(f"当前分支是 {current_branch}，正在切换到 {target_branch}...")
            if not git.checkout_branch(target_branch):
                logger.error(f"错误: 切换分支失败")
                return False
        
        # 检查工作目录是否干净
        if not git.is_workdir_clean():
        ...
        if pull_choice.upper() != "N":
            ...
            if git.checkout_branch(track_branch):
                if not git.pull_branch(upstream_remote, remote_branch):
        ...
        # 获取新提交列表
        logger.info(f"\n获取从 {last_commit[:7]}..{track_branch} 的新提交...")
        new_commits = git.get_new_commits(last_commit, track_branch)
        ...
        for i, commit_id in enumerate(new_commits, 1):
            ...
            parent_count = git.get_parent_count(commit_id)
            
            # 获取完整提交消息
            full_msg = git.get_commit_message(commit_id)
...
def _handle_normal_commit(commit_id, full_msg):
    ...
    if not git.commit_with_message(full_msg):
...
def _handle_merge_commit(commit_id, full_msg):
    """处理 merge commit"""
    p1 = git.get_first_parent(commit_id)
    p2 = git.get_second_parent(commit_id)
    
    # 检查是否为空合并
    if git.is_empty_merge(commit_id, p1):
        logger.info("  空合并，跳过")
        return
    
    # 检查是否有冲突解决
    if git.has_conflict_resolution(commit_id, p1, p2):
    ...
    if need_squash:
        ...
        for sub in subs:
            sub_msg = git.get_commit_message(sub)
            sub_msg += f"\n\ncherry picked from commit {sub}"
            
            if not git.cherry_pick_commit(sub):
            ...
            if not git.commit_with_message(sub_msg):
...
def _squash_commit(commit_id, full_msg):
    """Squash 提交"""
    if not git.run_cmd(f"git read-tree --reset -u {commit_id}"):
